...
class Model_IORenderer: NSObject, MTKViewDelegate {
    ...
    init(view: MTKView, device: MTLDevice) {
        ...
        // 球体
         coneModel = DvrSphere(device: device, xExtent: 200, yExtent: 200, zExtent: 200, uTesselation: 64, vTesselation: 64)
        ...
        do {
            coneTexture = try makeTexture(device: device, name: "bluemarble")
        } catch {
            fatalError("Error: Can not load texture")
        }
    ...
    func update(view: Model_IOMetalView, drawableSize:CGSize) {
        ...
        cubeModel.metallicTransform.update(camera: camera, transformer: {
            return GLKMatrix4Translate(view.arcBall.rotationMatrix, 0, -180, 0)
        })
        
        // cone model
        coneModel.metallicTransform.update(camera: camera, transformer: {
            return   GLKMatrix4Translate(view.arcBall.rotationMatrix, 0, 100, 0)
        })
